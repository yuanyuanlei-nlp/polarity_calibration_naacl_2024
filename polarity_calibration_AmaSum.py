...
with open("./AmaSum/target_polarity_score/target_polarity_score_save_dict.json", "r") as f:
    target_polarity_score_save_dict = json.load(f)

with open("./AmaSum/target_polarity_score/target_polarity_score_logits_save_dict.json", "r") as f:
    target_polarity_score_logits_save_dict = json.load(f)



''' sentiment model used to calculate polarity reward '''

sentiment_tokenizer = AutoTokenizer.from_pretrained("FacebookAI/roberta-base")
sentiment_model = AutoModelForSequenceClassification.from_pretrained("FacebookAI/roberta-base")
sentiment_model.load_state_dict(torch.load("./saved_models/sentiment_model.ckpt", map_location=device)) # load sentiment model
...
def evaluate(model, eval_dataloader, verbose):
    ...
    for step, batch in enumerate(eval_dataloader):
        ...
        sentiment_inputs = sentiment_tokenizer(sent_tokenize(generated_text), padding=True, truncation=True, max_length=512, return_tensors="pt")
        sentiment_inputs = sentiment_inputs.to(device)
        sentiment_outputs = sentiment_model(**sentiment_inputs)
        polarity_score = torch.mean(softmax_1(sentiment_outputs.logits)[:, 1]).item()
...
model.load_state_dict(torch.load("./saved_models/base_model_best_AmaSum.ckpt", map_location=device)) # load base summarizer
model.cuda()
    ...
    for batch in train_dataloader:
        ...
        if total_num_batch % valid_steps == 0:
            ...
            if polarity_distance < best_polarity_distance:
                torch.save(model.state_dict(), "./saved_models/calibrated_model_best_polarity_distance_AmaSum.ckpt")
                best_polarity_distance = polarity_distance

            if metric > best_metric:
                torch.save(model.state_dict(), "./saved_models/calibrated_model_best_metric_AmaSum.ckpt")
                best_metric = metric
        ...
        if baseline_generated_text == "" or generated_text == "":
            ...
            sentiment_inputs = sentiment_tokenizer(sent_tokenize(baseline_generated_text), padding=True, truncation=True, max_length=512, return_tensors="pt")
            sentiment_inputs = sentiment_inputs.to(device)
            sentiment_outputs = sentiment_model(**sentiment_inputs)
            baseline_polarity_score_logits = torch.mean(sentiment_outputs.logits[:, 1]).item()
            baseline_polarity_score = torch.mean(softmax_1(sentiment_outputs.logits)[:, 1]).item()

            sentiment_inputs = sentiment_tokenizer(sent_tokenize(generated_text), padding=True, truncation=True, max_length=512, return_tensors="pt")
            sentiment_inputs = sentiment_inputs.to(device)
            sentiment_outputs = sentiment_model(**sentiment_inputs)
            polarity_score_logits = torch.mean(sentiment_outputs.logits[:, 1]).item()
            polarity_score = torch.mean(softmax_1(sentiment_outputs.logits)[:, 1]).item()
        ...
        with accelerator.accumulate(model):
            ...
            loss = max(0, (polarity_reward - baseline_polarity_reward) / abs(polarity_reward - baseline_polarity_reward)) * (polarity_loss + similarity_loss + fluency_loss + task_loss)
...
model.load_state_dict(torch.load("./saved_models/calibrated_model_best_metric_AmaSum.ckpt", map_location=device))
...
model.load_state_dict(torch.load("./saved_models/calibrated_model_best_polarity_distance_AmaSum.ckpt", map_location=device))
rmse, mae, polarity_distance, rouge1, rouge2, rougeL, rougeLsum, rouge, metric = evaluate(model = model, eval_dataloader = test_dataloader, verbose = 1)
